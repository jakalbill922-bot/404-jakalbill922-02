...
class Gaussian:
    ...
    def transform_data(self, T, R):
        """
        Áp dụng biến đổi cứng toàn cục x' = R x + T cho toàn bộ Gaussian.
        - T: (3,) np.ndarray hoặc torch.Tensor
        - R: (3,3) np.ndarray hoặc torch.Tensor (ma trận quay trực chuẩn)
        Lưu ý: AABB giữ nguyên; _xyz sẽ được cập nhật lại theo AABB cũ.
        """
        if self._xyz is None:
            raise ValueError("Chưa có dữ liệu xyz trong Gaussian (self._xyz is None).")
        # chuẩn hóa input về torch trên đúng device
        ...
        with torch.no_grad():
            # 1) transform vị trí trong world-space rồi ghi ngược về _xyz (theo aabb)
            xyz_world = self.get_xyz  # (N,3) = _xyz * aabb[3:] + aabb[:3]
            xyz_world_new = xyz_world @ R.T + T  # (N,3)
            self.from_xyz(xyz_world_new)  # cập nhật _xyz theo aabb hiện tại

            # 2) transform orientation nếu có (_rotation có dạng quaternion + bias)
            if self._rotation is not None:
                ...
                self.from_rotation(q_new)  # ghi lại theo định dạng ẩn (_rotation = q - bias)
        return self
    ...
    def copy(self):
        ...
        # Tạo instance mới với cùng tham số khởi tạo
        new_gaussian = Gaussian(**self.init_params)

        # Chuyển về đúng device
        device = self.device

        # Sao chép tensor độc lập (clone + detach + to(device))
        if self._xyz is not None:
        ...
        # Sao chép aabb và biases để tránh reference sharing
